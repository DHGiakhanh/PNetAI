...
export const Register = () => {
  ...
  const [assignedSaleCode, setAssignedSaleCode] = useState('');
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const response = await authService.register({
        ...
        role: formData.role,
        saleCode:
          formData.role === 'service_provider' && formData.saleCode.trim()
            ? formData.saleCode.trim()
            : undefined
      });
      ...
      setAssignedSaleCode(response?.user?.saleCode || '');
      setSuccess(true);
    } catch (err: any) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-r from-warm via-warm to-cream flex items-center justify-center px-4">
        <div className="max-w-md w-full bg-white/60 backdrop-blur-xl rounded-3xl shadow-xl p-8 border border-sand text-center">
          ...
          {formData.role === 'service_provider' && assignedSaleCode && (
            <p className="text-sm text-brown mb-2 flex items-center justify-center gap-1">
              <Check className="w-4 h-4" /> Sale ID linked: {assignedSaleCode}
            </p>
          )}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-warm via-warm to-cream flex items-center justify-center px-4 py-12">
      <div className="max-w-6xl w-full bg-white/60 backdrop-blur-xl shadow-xl rounded-3xl overflow-hidden flex flex-col lg:flex-row">
        ...
        <div className="w-full lg:w-1/2 px-8 sm:px-12 py-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {formData.role === 'service_provider' ? (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Sale ID <span className="text-gray-400">(optional)</span>
                </label>
                <input
                  type="text"
                  name="saleCode"
                  placeholder="Enter sale ID (example: SALE001) or leave blank"
                ...
                />
                <p className="mt-1 text-xs text-gray-400">
                  If empty, the system will auto-assign a sale representative evenly.
                </p>
              </div>
            ) : null}
            ...
            <p className="text-xs text-gray-400">
              ...
              <span className="text-brown cursor-pointer">Terms of Service</span>{" "}
              and{" "}
              <span className="text-brown cursor-pointer">Privacy Policy</span>.
            </p>

            <button
              type="submit"
              disabled={loading}
              className="w-full mt-2 bg-brown hover:bg-brown-dark disabled:bg-sand text-white font-semibold py-3 rounded-full shadow-lg shadow-brown/20 text-sm transition"
            >
          ...
          <p className="mt-6 text-sm text-gray-500">
            ...
            <Link to="/login" className="text-brown font-medium">
              Log In
            </Link>
          </p>
        ...
        <div className="hidden lg:flex w-full lg:w-1/2 bg-gradient-to-br from-sand via-warm to-sage items-center justify-center relative p-10">
          ...
          <div className="relative w-96 h-[520px] rounded-[2.2rem] overflow-hidden shadow-2xl bg-sand rotate-3">
            <img
